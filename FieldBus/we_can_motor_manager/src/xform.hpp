...
inline double Abs(double a){if(a>=0.0)return a;else return -a;}
  ...
  */
  ...
  { // angle should even work in wrap around case!  
    return Pose2D(xform::random() * (angle.max - angle.min) + angle.min,
                  Vector2<double>(xform::random() * (x.max - x.min) + x.min,
                                  xform::random() * (y.max - y.min) + y.min));
  }
  bool operator > (const Pose2D& other)const
  {
...
public:
	...
	*/
	...
	/**
	* returns an entry and update it by user
	*/
	V& toUpdate () 
	{
		pos++;
	...
	*/
	...
	/** 
	* set the zero index pose
	* \offset relative to current pos
	*/
