...
//  }
  ...
  // check cmd state and send the no empty cmd

//  static uint8_t read_pid_count        = 0;
//  static uint8_t set_pid_count         = 0;
//  static uint8_t set_max_speded_count  = 0;
//  static uint8_t circle_position_count = 0;
//  static uint8_t speed_count           = 0;
//  static uint8_t stop_control_count    = 0;
//  SendReadPIDFromBusCmd(read_pid_count++);
//  SendSetPIDToRAMCmd(set_pid_count++);
//  SendSetMaxSpeedCmd(set_max_speded_count++);
//  SendCircleCPositionControlCmd(circle_position_count++);
//  SendSpeedControldCmd(speed_count++);
//  SendStopControlCmd(stop_control_count++);
}
...
//void PanTiltMotorManager::SetMaxSpeedControl(uint8_t motor_id, double max_speed) {
////  PanTiltMotorControlCmd cur_motor_cmd = motor_map_[motor_id]->getFrameControlCmd();
////  cur_motor_cmd.SetMaxSpeed(max_speed);
...
//    std::cout << config_path << std::endl;
  YAML::Node cfgs = YAML::LoadFile(config_path);
